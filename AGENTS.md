# AGENTS.md — psygil-website

Operating context and design brief for every Codex session working on this repo.
...
- Never add `Co-Authored-By: Codex` or similar watermarks to commits.
...
- Anthropic `frontend-design` skill — installed at `~/.Codex/skills/frontend-design/SKILL.md`.
...
- Emil Kowalski — https://emilkowal.ski (public interaction-design writing; not fetchable from Codex's sandbox).
- Impeccable — https://impeccable.style (not fetchable from Codex's sandbox).
- shadcn/ui — https://ui.shadcn.com/docs/skills (not fetchable from Codex's sandbox).
- UI-Skills — https://www.ui-skills.com/skills/{interaction-design,interface-design,web-design-guidelines} (not fetchable from Codex's sandbox).

For the fetch-blocked sources, paste the relevant skill content into `DESIGN_PRINCIPLES.md` in this repo and Codex will merge it into this file on next session.
