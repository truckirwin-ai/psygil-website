...
**Status:** note 2026-07-10: the founder reworked `download.html` yesterday for pricing v2 (roadmap teaser removed, founder-rate section added, Practice card replaced by the Stripe pricing table). The Jul 9 diff carries no "non-PHI" line on the Solo surface, so this item is still open, and the copy target has moved: the non-PHI line now belongs in the founder-rate/pricing-table context, not the old Solo card. The repo is active again, so the earlier hold (waiting for a live repo ruling) is lifted; a fresh 45-minute pass against the new page layout would close this. Live checkout raises the stakes: a Solo buyer can now pay without seeing the non-PHI boundary.
...
**Status:** likely resolved in practice 2026-07-09. The founder shipped seat-band Practice pricing on the boreas-website repo (up to 10 seats $2,500, 11 to 15 seats $3,600, 16 to 20 seats $4,500 per month; commits d46f091 and cc5b9a0). That is a third structure neither original option named: flat-rate bands, roles unspecified. One word from the founder confirms the shipped structure as the decision and this closes.
**Acceptance:** A documented decision on whether Practice tier is "3 clinician seats" or "1 lead + 2 support seats (psychometrician/admin/second evaluator)" per Clinician seat finding. Best resolved after the first 5 customer discovery interviews land. Amended 2026-07-10: founder confirmation of the shipped seat-band structure satisfies this.
